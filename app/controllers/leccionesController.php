...
class leccionesController extends Controller {
  ...
  // Validar rol
    ...
    $fecha_ini    = clean($_POST["feha_inicial"]);
    $fecha_max    = clean($_POST["fecha_max"]);
    $status       = clean($_POST["status"]); 

    // Validar que el profesor exista
    if (!$profesor = profesorModel::by_id($id_profesor)) {
      throw new Exception('El profesor de la leccion no existe en la base de datos.');
    }
    ...
    // Validar el nomore del usuario
    if (strlen($titulo) < 5) {
      throw new Exception('Ingresa un titulo mayor a 5 caracteres.');
    }

    // Validar que el url del video
    if (!filter_var($video, FILTER_VALIDATE_URL) && !empty($video)) {
      throw new Exception('Ingresa una URL de video valida.');
    }
    ...
    if (!$id = leccionModel::add(leccionModel::$t1, $data)) {
      ...
      Flasher::new(sprintf('Nueva leccion titulada <b>%s</b> agregada con éxito para la materia <b>%s</b>.', add_ellipsis($titulo, 50), $materia['nombre']), 'success');
  ...
  function post_editar()
  {
    try { 
      ...
      Flasher::new(sprintf('Leccion titulada <b>%s</b> actualizada con exito.', add_ellipsis($titulo, 50)), 'success');
  ...
  function borrar($id)
  {
    try {
      ...
      // Validar que exista la lección
      if (!$leccion = leccionModel::by_id($id)) {
        throw new Exception('No existe la lección seleccionada.', 'danger');
      }
